...
const PERMISSION_OPTIONS = [
  ...
  { value: 'sales.credit.override', label: 'Outrepasser credit client' },
  { value: 'clients.credit.view', label: 'Voir scoring credit client' },
...
export function UsersPage() {
  ...
  const users = useQuery({
    queryKey: ['users'],
    queryFn: async () =>
      (await api.get<{ users: User[] }>('/users')).data.users,
  });
  ...
  const filteredUsers = useMemo(() => {
    ...
    return (users.data ?? []).filter((user) => {
      ...
      const franchiseName = user.franchiseId
        ? (franchisesById.get(user.franchiseId) ?? '')
        : '';
      const matchesSearch =
        !q ||
          ...
          <article key={userKey(user)} className="mobile-record-card space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div className="text-right">
                ...
                <div className="mt-1 font-medium text-surface-800">
                  {user.franchiseId
                    ? (franchisesById.get(user.franchiseId) ?? '-')
                    : '-'}
                </div>
              ...
              <div className="col-span-2">
                ...
                <div className="mt-1 font-medium text-surface-800">
                  {dateTime(user.lastLoginAt ?? undefined)}
                </div>
              </div>
            </div>
            <div className="grid gap-2 sm:grid-cols-2">
              ...
              {isActiveUser(user) && (
                <button
                  ...
                  onClick={() => deactivate.mutate(userKey(user))}
                >
              ...
              <tr key={userKey(user)}>
                ...
                <td className="td-action">
                  <div className="flex justify-end gap-2">
                    ...
                    {isActiveUser(user) && (
                      <button
                        className="btn-danger !min-h-[36px] !px-3 !py-1.5"
                        onClick={() => deactivate.mutate(userKey(user))}
                      >
                        <UserX className="h-4 w-4" />
                        Desactiver
...
function Avatar({ user, size = 'md' }: { user: User; size?: 'md' | 'lg' }) {
  ...
  return (
    <div
      className={`${sizeClass} flex-shrink-0 overflow-hidden rounded-lg border border-slate-200 bg-slate-100`}
    >
      {user.avatarPath ? (
        <img
          src={uploadUrl(user.avatarPath)}
          alt={user.fullName}
  ...
  const save = useMutation({
    mutationFn: async (values: EditValues) => {
      const payload: Record<string, unknown> = {
        ...values,
        franchiseId:
          scoped || optionalFranchise ? values.franchiseId || null : null,
      ...
      if (avatarFile) {
        ...
        await api.post(
          `/users/${userKey(response.data.user)}/avatar`,
          formData,
        );
      }
    },
      ...
      >
        ...
        <div>
          ...
          <select
            ...
            disabled={!scoped && !optionalFranchise}
          >
            <option value="">-</option>
        ...
        <div className="sm:col-span-2">
          ...
          <select className="input" {...register('managerId')}>
            ...
            {users
              .filter(
                (item) => userKey(item) !== (initial ? userKey(initial) : ''),
              )
              .filter(isActiveUser)
              .map((item) => (
          ...
          <p className="mt-1 text-xs text-surface-500">
            Conge: vendeur vers manager franchise, commercial vers directeur
            commercial, sinon manager direct.
          </p>
        </div>
        <div className="sm:col-span-2">
        ...
        <div className="sm:col-span-2 rounded-xl border border-surface-200 bg-surface-50 p-3">
          <div className="mb-3 text-sm font-semibold text-surface-900">
            ...
            {PERMISSION_OPTIONS.map((permission) => {
              ...
              return (
                <label
                  ...
                  className="grid grid-cols-[minmax(0,1fr)_130px] items-center gap-2 rounded-lg bg-white px-3 py-2 text-sm"
                >
                  ...
                  <select
                    ...
                    onChange={(event) => {
                      ...
                      if (event.target.value === 'grant')
                        nextGrants.add(permission.value);
                      if (event.target.value === 'revoke')
                        nextRevokes.add(permission.value);
