...
export function AuditPage() {
  ...
  const filterKey = [debouncedQ, action, entity, username, ip, fromDate, toDate, pageSize].join('|');
  ...
  const params = useMemo(
    ...
    [action, debouncedQ, entity, fromDate, ip, page, pageSize, toDate, username],
  );

  const audit = useQuery({
    queryKey: ['audit', params],
    queryFn: async () => (await api.get<AuditResponse>('/audit', { params })).data,
    placeholderData: (previous) => previous,
  });
  const rows = audit.data?.logs ?? [];
  const hasFilters = Boolean(q || action || entity || username || ip || fromDate || toDate);
  ...
  return (
    <>
      <PageHeader title="Journal d’audit" subtitle="Événements sensibles, filtres et traçabilité" />
      ...
      <section className="card mb-5 p-4">
        <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-surface-500">
              {audit.data?.meta.total ?? 0} événement(s) visible(s). Les refresh techniques d’échéances sont masqués.
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button type="button" className="btn-secondary !min-h-[40px] !px-3 !py-2" disabled={!hasFilters} onClick={resetFilters}>
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-8">
          ...
          <label>
            ...
            <input className="input font-mono" value={ip} onChange={(event) => setIp(event.target.value)} placeholder="127.0.0.1" />
          </label>
          <label>
        ...
        {rows.map((log) => (
          <article key={log._id} className="mobile-record-card space-y-3">
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="text-xs font-medium text-surface-500">{dateTime(log.createdAt)}</div>
                <div className="mt-1 font-semibold text-surface-900">{log.username ?? 'Système'}</div>
              </div>
              <span className="badge-info max-w-[56%] justify-center text-center">{log.action}</span>
            </div>
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div>
                <div className="text-surface-400">Entité</div>
                <div className="font-semibold text-surface-700">{log.entity ?? '—'}</div>
              </div>
              <div>
                <div className="text-surface-400">IP</div>
                <div className="font-mono font-semibold text-surface-700">{log.ip ?? '—'}</div>
              </div>
            </div>
            {log.details ? (
              <details className="rounded-lg bg-surface-50 p-3 text-xs">
                <summary className="cursor-pointer font-semibold text-surface-600">Détails</summary>
                <pre className="mt-2 max-h-52 overflow-auto whitespace-pre-wrap font-mono text-surface-500">{detailText(log.details)}</pre>
              </details>
            ) : null}
          </article>
        ))}
        {!audit.isLoading && rows.length === 0 && (
          <div className="mobile-record-card text-sm text-surface-500">Aucun événement.</div>
        )}
      </section>
      ...
      <div className="card hidden overflow-x-auto lg:block">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.map((log) => (
              <tr key={log._id}>
                <td className="td whitespace-nowrap text-slate-500">{dateTime(log.createdAt)}</td>
                <td className="td">
                  <div className="font-semibold">{log.username ?? 'Système'}</div>
                  {log.userAgent && <div className="mt-1 max-w-48 truncate text-xs font-normal text-surface-400">{log.userAgent}</div>}
                </td>
                <td className="td"><span className="badge-info">{log.action}</span></td>
                ...
                <td className="td max-w-xl text-xs text-slate-500">
                  {log.details ? (
                    <details>
                      <summary className="cursor-pointer font-semibold text-surface-600">Voir détails</summary>
            ...
            {audit.isLoading && (
              <tr><td className="td text-slate-400" colSpan={6}>Chargement...</td></tr>
            )}
            {!audit.isLoading && rows.length === 0 && (
