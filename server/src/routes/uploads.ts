...
function hasPermission(req: Express.Request, permission: Permission) {
  const user = req.user;
  return Boolean(
    user && isPermissionGranted(user.role, permission, user.customPermissions),
  );
}
...
async function assertSensitiveUploadAccess(
  req: Express.Request,
  bucket: string,
  uploadPath: string,
) {
  ...
  if (bucket === "product-images") {
    ...
    const product = await Product.findOne({ imagePath: uploadPath })
      .select("_id active")
      .lean();
  ...
  if (bucket === "user-avatars") {
    const owner = await User.findOne({ avatarPath: uploadPath })
  ...
  if (bucket === "treasury-docs" || bucket === "treasury-receipts") {
    ...
    const pathField =
      bucket === "treasury-docs" ? "attachmentPath" : "receiptPath";
    ...
    if (
      !flow.isCentralCashbox &&
      user.franchiseId &&
      flow.franchiseId?.toString() === user.franchiseId
    )
      return;
    throw forbidden();
  }
  ...
  if (bucket === "installment-receipts") {
    ...
    const installment = await Installment.findOne({ receiptPath: uploadPath })
      .select("franchiseId")
      .lean();
    ...
    if (
      user.franchiseId &&
      installment.franchiseId?.toString() === user.franchiseId
    )
      return;
    throw forbidden();
  }
  ...
  if (bucket === "reception-ocr") {
    ...
    if (
      user.franchiseId &&
      reception.franchiseId?.toString() === user.franchiseId
    )
      return;
    throw forbidden();
  }
  ...
  if (bucket === "network-point-docs") {
    ...
    if (
      ...
      point.commercialId?.toString() === user.sub
    )
      return;
    if (
      user.role === "commercial" &&
      (await canAccessCommercialZone(user.sub, point.zoneId))
    )
      return;
    if (user.franchiseId && point.franchiseId?.toString() === user.franchiseId)
      return;
    throw forbidden();
  }

  if (bucket === "client-docs") {
    if (!hasPermission(req, "clients.credit.view")) throw forbidden();
    const client = await Client.findOne({
      $or: [
        { "documents.cinImagePath": uploadPath },
        { "documents.payslipPath": uploadPath },
        { "documents.proofOfAddressPath": uploadPath },
        { "documents.signedAgreementPath": uploadPath },
      ],
    })
      .select("franchiseId")
      .lean();
    if (!client) throw notFound("File not found");
    if (isGlobalRole(user.role)) return;
    if (user.franchiseId && client.franchiseId?.toString() === user.franchiseId)
      return;
    throw forbidden();
  }
}
