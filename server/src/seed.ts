...
async function resetCollections() {
  await Promise.all([
    ...
    Service.deleteMany({}),
    NetworkPointAllocation.deleteMany({}),
    CommercialZone.deleteMany({}),
...
async function seed() {
  ...
  const superadminHash = await bcrypt.hash('superadmin', env.BCRYPT_ROUNDS);
  ...
  const franchises = await Franchise.insertMany([
    ...
    {
      name: 'ASEL Mobile Cite Intileka',
      address: 'Cite Intileka, Tunis',
      phone: '',
      manager: 'Intileka',
      gps: { lat: 36.839, lng: 10.101 },
      active: true,
    },
    {
      name: 'ASEL Mobile Marsa',
      address: 'La Marsa, Tunis',
      phone: '+216 47 001 566',
      manager: 'Sirine Hadiji',
      gps: { lat: 36.8787, lng: 10.3249 },
      active: false,
    },
  ]);
  ...
  const intileka = must(franchises[3], 'Intileka franchise missing');
  ...
  const categories = await Category.insertMany([
    ...
    { name: 'Tablets', description: 'Tablets and large-screen devices' },
    { name: 'Scooters & Mobility', description: 'Electric scooters and mobility products' },
  ]);
  ...
  const suppliers = await Supplier.insertMany([
    ...
    { name: 'STIC', phone: '+216 90 487 380', email: '', address: 'Kram, Tunis' },
    { name: 'VLT Motors', phone: '+216 70 654 300', email: '', address: 'Akouda, Sousse' },
  ]);
  ...
  const products = await Product.insertMany([
    ...
    {
      name: 'Type C iPhone 15+ 1m',
      categoryId: must(categoryByName.get('Cables'), 'Category Cables missing')._id,
      supplierId: must(supplierByName.get('STIC'), 'Supplier STIC missing')._id,
      brand: 'INKAX',
      reference: 'CK-123-TC',
      barcode: '',
      purchasePrice: 10,
    ...
    {
      name: 'Apple Original USB-C Cable 1m',
      categoryId: must(categoryByName.get('Cables'), 'Category Cables missing')._id,
      supplierId: must(supplierByName.get('STIC'), 'Supplier STIC missing')._id,
      brand: 'Apple',
      reference: 'MQKJ3ZM/A',
    ...
    {
      name: 'Smart Charger 2.4A',
      categoryId: must(categoryByName.get('Chargeurs'), 'Category Chargeurs missing')._id,
      supplierId: must(supplierByName.get('STIC'), 'Supplier STIC missing')._id,
      brand: 'ASPOR',
      reference: 'A818',
      barcode: '6971393452049',
      purchasePrice: 10,
      sellPrice: 30,
      lowStockThreshold: 5,
      active: true,
    ...
    {
      name: 'Metal Car Mount',
      categoryId: must(categoryByName.get('Car Accessories'), 'Category Car Accessories missing')._id,
      supplierId: must(supplierByName.get('Actelo'), 'Supplier Actelo missing')._id,
      brand: 'VIDVIE',
      reference: 'HC1508',
      barcode: '',
    ...
    {
      name: 'Wireless Speaker Bluetooth 5W',
      categoryId: must(categoryByName.get('Speakers'), 'Category Speakers missing')._id,
      supplierId: must(supplierByName.get('Mokhtar Trading'), 'Supplier Mokhtar missing')._id,
      brand: 'Wireless',
      reference: 'JZ-200',
      barcode: '',
    ...
    {
      name: 'Realme C61 8/256',
      categoryId: must(categoryByName.get('Phones'), 'Category Phones missing')._id,
      supplierId: must(supplierByName.get('Actelo'), 'Supplier Actelo missing')._id,
      brand: 'Realme',
      reference: 'GPS50-RC61',
      barcode: '',
      purchasePrice: 516,
      sellPrice: 570,
      lowStockThreshold: 2,
    ...
    {
      name: 'Spider Max Scooter 500W',
      categoryId: must(categoryByName.get('Scooters & Mobility'), 'Category Scooters missing')._id,
      supplierId: must(supplierByName.get('VLT Motors'), 'Supplier VLT Motors missing')._id,
      brand: 'Spider',
      reference: 'SPIDER-MAX-500W',
      barcode: '',
      purchasePrice: 900,
      sellPrice: 1250,
      lowStockThreshold: 1,
    ...
    {
      name: 'Wolf X2 Scooter 500W',
      categoryId: must(categoryByName.get('Scooters & Mobility'), 'Category Scooters missing')._id,
      supplierId: must(supplierByName.get('VLT Motors'), 'Supplier VLT Motors missing')._id,
      brand: 'Wolf',
      reference: 'WOLF-X2-500W',
      barcode: '',
      purchasePrice: 1700.01,
      sellPrice: 2250,
      lowStockThreshold: 1,
      active: true,
    },
    {
      name: 'Tablette Infinix 8/256',
      categoryId: must(categoryByName.get('Tablets'), 'Category Tablets missing')._id,
      supplierId: must(supplierByName.get('Infogenie'), 'Supplier Infogenie missing')._id,
      brand: 'Infinix',
      reference: 'TAB-INFINIX-8256',
      barcode: '',
      purchasePrice: 425,
      sellPrice: 550,
      lowStockThreshold: 2,
    ...
    {
      name: 'Coque Silicon Telephone',
      categoryId: must(categoryByName.get('Cases'), 'Category Cases missing')._id,
      supplierId: must(supplierByName.get('STIC'), 'Supplier STIC missing')._id,
      brand: 'Generic',
      reference: 'SILICON-CASE',
      barcode: '',
      purchasePrice: 6.5,
      sellPrice: 20,
      lowStockThreshold: 10,
  ...
  const users = await User.insertMany([
    ...
    },
    {
      username: 'intileka',
      passwordHash: sharedHash,
      fullName: 'Intileka Manager',
      role: 'franchise',
      franchiseId: intileka._id,
      active: true,
    },
    {
    ...
    {
      username: 'commercial_mourouj',
      passwordHash: sharedHash,
      fullName: 'Commercial Mourouj',
      role: 'commercial',
      franchiseId: mourouj._id,
      active: true,
    },
    {
      username: 'siege_employee',
      passwordHash: sharedHash,
      fullName: 'Employe Siege',
      role: 'siege_employee',
      franchiseId: null,
      active: true,
    },
  ...
  const commercialMourouj = must(userByUsername.get('commercial_mourouj'), 'commercial_mourouj missing');
  const [mouroujZone, soukraZone] = await CommercialZone.insertMany([
    {
      name: 'Zone Commercial Mourouj',
      color: '#2563eb',
      franchiseId: mourouj._id,
      assignedCommercialIds: [commercialMourouj._id],
      polygon: [
        { lat: 36.715, lng: 10.185 },
        { lat: 36.715, lng: 10.245 },
        { lat: 36.755, lng: 10.245 },
        { lat: 36.755, lng: 10.185 },
      ],
      note: 'Zone demo liee au commercial Mourouj',
      createdBy: must(userByUsername.get('manager'), 'manager user missing')._id,
    },
    {
      name: 'Zone Commercial Soukra Dar Fadhal',
      color: '#16a34a',
      franchiseId: soukra._id,
      assignedCommercialIds: [commercialMourouj._id],
      polygon: [
        { lat: 36.858, lng: 10.236 },
        { lat: 36.858, lng: 10.272 },
        { lat: 36.883, lng: 10.272 },
        { lat: 36.883, lng: 10.236 },
      ],
      note: 'Zone legacy Dar Fadhal / Soukra rattachee au commercial pour la demo terrain',
      createdBy: must(userByUsername.get('manager'), 'manager user missing')._id,
    },
  ]);
  const zoneMourouj = must(mouroujZone, 'Mourouj zone missing');
  const zoneSoukra = must(soukraZone, 'Soukra zone missing');
  ...
  const networkPoints = await NetworkPoint.insertMany([
    ...
    },
    {
      ...
      franchiseId: soukra._id,
      commercialId: commercialMourouj._id,
      zoneId: zoneSoukra._id,
      commissionPct: 5,
      active: true,
      createdBy: must(userByUsername.get('manager'), 'manager user missing')._id,
    ...
    },
    {
      ...
      type: 'franchise',
      status: 'actif',
      address: 'Mourouj, Ben Arous',
      city: 'Mourouj',
      governorate: 'Ben Arous',
      phone: '+216 52 123 456',
      responsible: 'Gerant Mourouj',
      gps: { lat: 36.734, lng: 10.1547 },
      franchiseId: mourouj._id,
      commercialId: commercialMourouj._id,
      zoneId: zoneMourouj._id,
      commissionPct: 0,
      active: true,
      createdBy: must(userByUsername.get('manager'), 'manager user missing')._id,
      contactDate: daysAgo(90),
      contractDate: daysAgo(88),
      activationDate: daysAgo(80),
    },
    {
      name: 'ASEL Mobile Soukra',
      type: 'franchise',
      ...
      city: 'Soukra',
      governorate: 'Ariana',
      phone: '+216 47 001 500',
      responsible: 'Gerant Soukra',
      gps: { lat: 36.867179, lng: 10.250789 },
      franchiseId: soukra._id,
      commercialId: commercialMourouj._id,
      zoneId: zoneSoukra._id,
      commissionPct: 0,
    ...
    },
    {
      ...
      governorate: 'Ariana',
      responsible: 'Fedi',
      gps: { lat: 36.863641, lng: 10.241772 },
      franchiseId: soukra._id,
      commercialId: commercialMourouj._id,
      zoneId: zoneSoukra._id,
      internalNotes: 'Asel Pay actif, contrat pas encore signe',
      commissionPct: 0,
      active: true,
      createdBy: must(userByUsername.get('manager'), 'manager user missing')._id,
      contactDate: daysAgo(34),
    },
    {
      name: 'Boutique Reparation Telephones',
      type: 'activation_recharge',
      status: 'actif',
      address: 'Dar Fadhal',
      city: 'Dar Fadhal',
      governorate: 'Ariana',
      responsible: 'Aymen / Hajer',
      gps: { lat: 36.866814, lng: 10.242275 },
      franchiseId: soukra._id,
      commercialId: commercialMourouj._id,
      zoneId: zoneSoukra._id,
      commissionPct: 0,
      active: true,
      createdBy: must(userByUsername.get('manager'), 'manager user missing')._id,
      contactDate: daysAgo(32),
    },
    {
      name: 'Librairie Dar Fadhal',
      type: 'activation_recharge',
      status: 'actif',
      address: 'Dar Fadhal',
      city: 'Dar Fadhal',
      governorate: 'Ariana',
      gps: { lat: 36.863008, lng: 10.246872 },
      franchiseId: soukra._id,
      commercialId: commercialMourouj._id,
      zoneId: zoneSoukra._id,
      commissionPct: 0,
      active: true,
      createdBy: must(userByUsername.get('manager'), 'manager user missing')._id,
      contactDate: daysAgo(30),
    },
    {
      name: 'Helmi',
      type: 'recharge',
      status: 'prospect',
      address: 'Devant hawet, Dar Fadhal',
      city: 'Dar Fadhal',
      governorate: 'Ariana',
      responsible: 'Helmi',
      gps: { lat: 36.865784, lng: 10.251552 },
      franchiseId: soukra._id,
      commercialId: commercialMourouj._id,
      zoneId: zoneSoukra._id,
      internalNotes: 'Boutique 3attar',
      commissionPct: 0,
      active: true,
      createdBy: must(userByUsername.get('manager'), 'manager user missing')._id,
      contactDate: daysAgo(20),
    ...
    {
      name: 'Boutique Weswes',
      type: 'recharge',
      status: 'prospect',
      address: 'Soukra',
      city: 'Soukra',
      governorate: 'Ariana',
      gps: { lat: 36.870188, lng: 10.248721 },
      franchiseId: soukra._id,
      commercialId: commercialMourouj._id,
      zoneId: zoneSoukra._id,
      commissionPct: 0,
      active: true,
      createdBy: must(userByUsername.get('manager'), 'manager user missing')._id,
      contactDate: daysAgo(18),
    },
    {
      name: 'Librairie Imtiaz',
      type: 'activation_recharge',
      status: 'prospect',
      address: 'Soukra',
      city: 'Soukra',
      governorate: 'Ariana',
      gps: { lat: 36.878119, lng: 10.267979 },
      franchiseId: soukra._id,
      commercialId: commercialMourouj._id,
      zoneId: zoneSoukra._id,
      commissionPct: 0,
    ...
    },
    {
      ...
      city: 'Soukra',
      governorate: 'Ariana',
      responsible: 'Ghassen Jridi',
      gps: { lat: 36.87935, lng: 10.26107 },
      franchiseId: soukra._id,
      commercialId: commercialMourouj._id,
      zoneId: zoneSoukra._id,
      commissionPct: 0,
      active: true,
      createdBy: must(userByUsername.get('manager'), 'manager user missing')._id,
      contactDate: daysAgo(16),
    },
    {
      name: 'Fedi Hamdi',
      type: 'activation_recharge',
      status: 'actif',
      address: 'Dar Fadhal, Soukra',
      city: 'Ariana',
      governorate: 'Ariana',
      phone: '56169182',
      responsible: 'Fedi Hamdi',
      gps: { lat: 36.86194, lng: 10.241604 },
      franchiseId: soukra._id,
      commercialId: commercialMourouj._id,
      zoneId: zoneSoukra._id,
      commissionPct: 10,
    ...
    },
    {
      ...
      city: 'Soukra',
      governorate: 'Ariana',
      gps: { lat: 36.870884, lng: 10.246861 },
      franchiseId: soukra._id,
      commercialId: commercialMourouj._id,
      zoneId: zoneSoukra._id,
      commissionPct: 0,
