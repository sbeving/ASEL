...
const bucketCompressionProfiles: Partial<
  Record<UploadBucket, CompressionProfile>
> = {
...
async function compressImageUpload(
  ...
  try {
    let pipeline = sharp(file.path, { failOn: "none" }).rotate().resize({
...
function compressUploadedImages(bucket: UploadBucket): RequestHandler {
  const profile =
    bucketCompressionProfiles[bucket] ?? defaultCompressionProfile;
...
export const clientDocumentUpload = createUploader(
  "client-docs",
  ["image/jpeg", "image/png", "image/webp", "application/pdf"],
  15 * 1024 * 1024,
  4,
